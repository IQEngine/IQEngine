...
def client():
    ...
    with mock.patch("helpers.datasource_access.check_access", new=check_access_mock("account", "container", None)):
        with mock.patch("helpers.authorization.required_roles", return_value=required_roles_mock()):
            with mock.patch("importer.all.import_all_from_env") as mock_i:
                with mock.patch("database.datasource_repo.check_access", new=check_access_mock("account", "container", None)):
                    with mock.patch("database.datasource_repo.get_current_user", new=get_current_user_mock()):
