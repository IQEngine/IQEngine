...
class markos_detector(Plugin):
    ...
    def rf_function(self, samples, job_id=None):
        ...
        for a in anots:
            td = {
                "start_samp": a["core:sample_start"],
                "end_samp": a["core:sample_start"] + a["core:sample_count"],
                "start_freq": a["core:freq_lower_edge"] - self.center_freq,
                "end_freq": a["core:freq_upper_edge"] - self.center_freq,
                "min_pwr": 123,
            }
            rects.append(td)
    ...
    for fft_loc in fft_locs:
        ...
        for idx in range(fft_size - n_floor_window_bins):
            ...
            if pwr < min_pwr:
                ...
                res = {
                    "start_samp": fft_loc,
                    "end_samp": fft_loc + fft_size,
                    "start_freq": freq_list[idx],
                    "end_freq": freq_list[idx + n_floor_window_bins],
                    "min_pwr": min_pwr,
                }
    return res
