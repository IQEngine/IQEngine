...
async def get_from_samples_cloud(samples_cloud: SamplesCloud):
    blob_client = get_blob(samples_cloud.account_name, samples_cloud.container_name, samples_cloud.file_path, samples_cloud.sas_token)

    if(samples_cloud.byte_length):
        download_stream = await asyncio.to_thread(
            blob_client.download_blob, samples_cloud.byte_offset, samples_cloud.byte_length
        )
    else:
