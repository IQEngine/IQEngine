  ...
  return (
    <div className="mx-32">
      ...
      <p className="text-lg">
        ...
        polarization in the filename, and if there are a lot of channels you will want to put them in their own
        directory. The SigMF standard does not specify how to perform data compression (e.g., gzip), which means
        software/tooling that uses SigMF is unlikely going to be able to perform the decompression for you, so you can
        choose to do compression however you like. The general recommendation for binary IQ files is before worrying
        about compression, first make sure that you are representing your RF recordings using the minimum bit depth,
        e.g., instead of float32 try int16 or even int8. If compressing your files leads to a large savings in space,
        then you probably could have achieved most of that gain by just using a lower bit depth in the first place. As
        such, it is most common to not use any data compression.
        <br></br>
        <br></br>
        ...
        <ul class="list-disc px-32 my-4">
          <li>
            ...
            (i), unsigned int (u) and examples include cf32_le for complex 32-bit floats (equivalent to numpy's
            complex64) and ci16_le for the common int16 using signed ints for each I and Q. The "c" means complex and
            the "_le" means little-endian, both of which are the standard for binary IQ files.
          </li>
          <li>
            <b>captures</b> - A list of "captures" where each capture includes the center (RF) frequency and
            sample_start which is the sample index the capture starts on. Typically recordings will only have 1 capture,
            so sample_start will be 0 and the center frequency will be the important piece of information. Multiple
            captures are most commonly seen when you frequency hop within the same recording. The optional field
            datetime indicates the date and time corresponding to the start of each capture, using the ISO 8601 format
            (e.g., 2022-03-17T16:43:30Z).
          </li>
          <li>
            <b>annotations</b> - A list of "annotations" where each annotation is like a bounding box in time and
        ...
        <pre className="text-sm mx-32 mb-4">
