...
import { useSyncAllDataSource } from '@/api/datasource/queries';

export const App = () => {
  const { instance } = useMsal();
  const activeAccount = instance.getActiveAccount();
  const syncAll = useSyncAllDataSource();
        ...
        <div className="container flex mb-16 sm:navbar mb-10 bg-base-100" id="topRow">
              ...
              <Feature flag={FeatureFlag.displayInternalBranding.name}>
                {config.data && config.data.internalBranding && (
                  <img
                    ...
                    className="md:w-82 md:h-30 mr-8 sm:w-60 sm:h-20"
                  />
                )}
              </Feature>
              <Logo />
          ...
          <div className="absolute right-0 py-2 mr-2 ">
            <ul className="flex-row flex mb-4 mt-0 space-x-5 text-sm font-medium">
              ...
              <Feature flag={FeatureFlag.allowRefreshing.name}>
                <li className="hidden md:block">
                  <div
                    className="text-primary text-base cursor-pointer"
                    onClick={(e) => {
                      e.preventDefault();
                      toast('Syncing all datasources', {
                        icon: '🔄',
                        className: 'font-bold',
                      });
                      syncAll();
