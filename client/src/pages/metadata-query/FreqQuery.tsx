    ...
    <div className="mb-10">
      ...
      {show && <div className="card bg-neutral text-neutral-content">
        <div className="card-body">
          ...
          <input data-testid="freq-lower" name="from" onChange={handleFreqChange} type="range" step={100} min={0} max={3000000000} value={freqRange.from}  className="range range-secondary w-full" />
          <h3 className="text-lg">
            To:
            <span className="ml-2 badge badge-md badge-success">{`${freqRange.to} Hz`}</span>
          </h3>
          <input data-testid="freq-upper" name="to" onChange={handleFreqChange} type="range" min={0} step={100} max={10000000000} value={freqRange.to} className="range range-secondary w-full" />
        </div>
      </div>}
    </div>
  )
