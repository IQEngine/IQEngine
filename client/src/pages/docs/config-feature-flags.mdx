...
* `useIQEngineOutReach`: When true this flag shows IQEngine outreach links, e.g. Discord. This is true as default.
...
* `allowRefreshing`: When set to true, there will be a button that appears in the top-right of every IQEngine page that lets you resync all of the backend datasources; both cloud and files local to backend (see IQENGINE_BACKEND_LOCAL_FILEPATH).  Anyone can click the button, there's no auth associated with it, so it's not used by IQEngine.org. 
