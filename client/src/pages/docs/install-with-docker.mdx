...
As part of IQEngine's CI/CD, Docker images are built and pushed to GitHub Container Registry.
...
3. Out of the same directory you put the .env file, run `docker run --env-file .env -v /path/to/myrecordings:/tmp/myrecordings -p 3000:3000 -d ghcr.io/iqengine/iqengine:pre` replacing `/path/to/myrecordings` with the local directory filled with SigMF recordings (this gets mounted to /tmp/myrecordings within the container, and the example .env already has that directory added to it)
