...
function getWrapper(children: any, metadata: SigMFMetadata) {
  return (
    <AllProviders>
...
describe('Annotation list component', () => {
  ...
  test('Columns display correctly', async () => {
    ...
    expect(await screen.findByText('Frequency Range')).toBeInTheDocument();
    expect(await screen.findByText('BW')).toBeInTheDocument();
  ...
  //   // Act
  //   render(<AnnotationList setCurrentFFT={() => {}} currentFFT={0}></AnnotationList>, {
  //     wrapper: ({ children }) => getWrapper(children, meta),
  //   });
