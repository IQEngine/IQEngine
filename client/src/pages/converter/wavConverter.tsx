import { useConvertWaveToSigmf } from '@/api/converter/wavToSigmf';
import React from 'react';
import fileDownload from 'js-file-download';

export const WavConverter = () => {
  const [selectedFile, setSelectedFile] = React.useState(null);

  const { refetch, isFetching, data } = useConvertWaveToSigmf(selectedFile);
  ...
  return (
    <div style={{ minWidth: '50%' }} className="flex flex-col ">
      ...
      {!isFetching && !data && (
        <>
          <label className="mb-2 inline-block text-neutral-200">Select .wav file</label>
          <input
            className="
