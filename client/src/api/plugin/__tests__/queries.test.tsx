...
import { useGetPlugins, useGetPlugin, useGetPluginParameters, useGetPluginDetailed } from '../queries';
...
describe('useGetPluginDetailed', () => {
  ...
  test('should be able to get detailed information from a plugin', async ({ expect }) => {
    const { wrapper } = useAllProviders();
    let expectedParametersPlugin1 = {
      param1: {
        title: 'Param1',
        type: 'string',
        default: 'default',
    ...
    nock('http://localhost:8000')
      ...
      .get(`/api/plugins/plugin1`)
      .reply(200, expectedParametersPlugin1);
    nock('http://localhost:8000')
      .defaultReplyHeaders({
        'access-control-allow-origin': '*',
        'access-control-allow-credentials': 'true',
      })
      .get(`/api/plugins/plugin2`)
      .reply(200, expectedParametersPlugin2);
    nock('http://localhost:8000')
      .defaultReplyHeaders({
        'access-control-allow-origin': '*',
        'access-control-allow-credentials': 'true',
    ...
    const { result } = renderHook(() => useGetPluginDetailed(pluginDefinition), {
      wrapper: wrapper,
    });

    const expectedPludingEndpoint = {
      name: 'gnuradio',
      url: 'http://localhost:8000/api/plugins/',
      plugins: {
        plugin1: expectedParametersPlugin1,
        plugin2: expectedParametersPlugin2,
      },
    };
    await waitFor(() => expect(result.current.isSuccess).toBe(true), { timeout: 1000 });
    await waitFor(() => expect(result.current.isFetching).toEqual(false), { timeout: 1000 });
    await waitFor(() => expect(result.current.data).toEqual(expectedPludingEndpoint), { timeout: 1000 });
  });
});
